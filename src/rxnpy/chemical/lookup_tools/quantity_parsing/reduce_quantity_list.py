...
from rxnpy.chemical.lookup_tools.quantity_parsing.utils import _quantity_approx_equal, _flatten_list
...
def quantity_list_to_dict(data_in: List[List[Quantity]]) -> Dict:
    ...
    for i, data in enumerate(data_in):
        if isinstance(data, Quantity):
            if any([_quantity_approx_equal(data, v["quantity"]) for v in data_dict.values()]):
                for k, v in data_dict.items():
                    if _quantity_approx_equal(data, v["quantity"]):
                        data_dict[k]["count"] += 1
            else:
...
def _select_from_data_dict(data_dict: Dict) -> Union[List[Quantity], Quantity]:
    ...
    single_list = _flatten_list(single_list)
